...
#======================================================================================#
filename = "../output/figure_3_model_calibration.pdf"
...
#======================================================================================#
# Create some plots... PLOT CASES MEAN---------
#======================================================================================#
filename = '../output/figure_5_imported_cases.pdf'
if(save_fig == TRUE){ pdf(filename,width=10,height=8)}
par(mfrow = c(4,1),mar=c(4,0,0,2),oma=c(0.5,4.5,2.5,2.5))
for(ss in 1:4){
  ymax = 1.3 *(max(imp.foi.mean.sampled[ss,]) + max(inf.foi.mean.sampled[ss,]))
  plot(Dates[time.sampled],inf.foi.mean.sampled[ss,] + imp.foi.mean.sampled[ss,], type = "l", col = color_array[ss], ylab = "", xlab = "", ylim = c(0,ymax),axes = F, lwd = 0)
  polygon(c(Dates[time.sampled],rev(Dates[time.sampled])), c(rep(0,length(imp.foi.mean.sampled[ss,])), rev(imp.foi.mean.sampled[ss,])), 
          col = '#00000050', border = 1)
  polygon(c(Dates[time.sampled],rev(Dates[time.sampled])), c(imp.foi.mean.sampled[ss,], rev(inf.foi.mean.sampled[ss,] + imp.foi.mean.sampled[ss,])),
          col = ci_color[ss], border = 1)
  legend('topleft', c(sprintf("Autochthonous DENV-%d",ss),sprintf("Infected visitors DENV-%d",ss)), col = c(color_array[ss],'#00000050'), pch = 15, pt.cex = 2, bty = 'n')
  yticks = seq(from = 0, by = 0.001, to = 0.005)
  axis(2)
  axis(1,at=as.Date(paste(2000:2011,'-01-01',sep='')),labels=as.character(2000:2011))
  if(ss==2){
